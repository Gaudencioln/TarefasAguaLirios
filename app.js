// === Dados & Utilitários ===
const LS_KEY = "al_tarefas_data_v2";
...
function bindAdmin(){
  ...
  document.getElementById("fileImportAll").addEventListener("change", ev => {
    handleImportFile(ev.target.files[0], null); // null = todos
    ev.target.value="";
  });
...
function renderAdmin(){
  ...
  state.data.workers.forEach(w => {
    ...
    card.querySelector(`[data-export="${w.id}"]`).addEventListener("click", () => {
      const mine = state.data.tasks.filter(t=>t.owner===w.id);
      const txt = serializeTasks(mine);
      downloadTxt(`tarefas_${slug(w.name)}.txt`, txt);
    });
    card.querySelector(`[data-import="${w.id}"]`).addEventListener("change", ev => {
      handleImportFile(ev.target.files[0], w.id);
      ev.target.value="";
    });
  });
...
function bindFunc(){
  document.getElementById("btnExportMine").addEventListener("click", () => {
    ...
    const mine = state.data.tasks.filter(t=>t.owner===state.currentWorker.id);
    const txt = serializeTasks(mine);
    downloadTxt(`tarefas_${slug(state.currentWorker.name)}.txt`, txt);
  });
  document.getElementById("fileImportMine").addEventListener("change", ev => {
    if(!state.currentWorker) return;
    handleImportFile(ev.target.files[0], state.currentWorker.id);
    ev.target.value="";
  });
}
...
// Formato de linha: TASK|ownerId|title|desc|due|status|doneAt|id
...
function handleImportFile(file, ownerId){
  if(!file) return;
  ...
  reader.onload = () => {
    ...
    lines.forEach(line => {
      ...
      const [_, owner, title, desc, due, status, doneAt, id] = parts;
      const task = {
        id: id || uid(),
        owner: ownerId || owner, // se ownerId foi passado, força import para esse funcionário
      ...
      if(!task.owner) return; // ignora sem dono
    ...
    saveData(state.data);
    alert(`Importadas ${imported} tarefa(s).`);
